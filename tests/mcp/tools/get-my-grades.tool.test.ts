...
describe('get_my_grades tool', () => {
  it('returns compact summary with letter grades', async () => {
    ...
    const r = await handleGetMyGrades({ gradeRepo: repo, output: testOutputContext() }, { course_id: 101 });
  ...
  it('returns detailed when format=detailed', async () => {
    const repo = new FakeGradeRepository(new Map([[101, [grade(1, 'Exam', 87)]]]));
    const r = await handleGetMyGrades({ gradeRepo: repo, output: testOutputContext() }, { course_id: 101, format: 'detailed' });
  ...
  it('handles empty grades gracefully', async () => {
    ...
    const r = await handleGetMyGrades({ gradeRepo: repo, output: testOutputContext() }, { course_id: 999 });
